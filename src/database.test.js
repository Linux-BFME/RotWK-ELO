import { getScoreFromDb, getScoresFromDb, updateScoreInDb, updateScoresInDb } from "./database.js";
...
test('Write invalid value (number below minElo) to database then expect the value to remain unchanged', done => {
  jest.spyOn(console, 'warn').mockImplementation(() => {});
...
test('Write invalid value (number over maxElo) to database then expect the value to remain unchanged', done => {
  jest.spyOn(console, 'warn').mockImplementation(() => {});
...
test('Write multiple values to database with updateScoresInDb then expect to see those values with getScoresFromDb', done => {
  updateScoresInDb([{ name: "test6", score: 50 }, { name: "test7", score: 100 }]).then(() => {
